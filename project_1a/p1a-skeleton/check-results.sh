...
< "$rundir/tests-$prog" xargs -n1 -I'{}' bash -c "script -q -e -c \"'$rundir/check-result.sh' '$prog' {} '$resultdir'\" /dev/null >/dev/null && echo '$prog $numthreads $numkeys {} $numiter: PASS' || echo '$prog $numthreads $numkeys {} $numiter: FAIL'"
