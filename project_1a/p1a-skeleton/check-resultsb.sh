...
< "$rundir/tests-$prog" xargs -n1 -I'{}' bash -c "'$rundir/check-result.sh' '$prog' {} '$resultdir' >/dev/null 2>&1 && echo '$prog $numthreads $numkeys {} $numiter: PASS' || echo '$prog $numthreads $numkeys {} $numiter: FAIL'"
