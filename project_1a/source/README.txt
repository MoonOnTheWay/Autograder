This directory contains the solution to project 1a
...
- noyield-test  - check whether NOYIELD macro was defined in hashchain.cc
- pthread-rwlock-test - check whether rwlock.cc contains the text pthread_rwlock
Clean:
...
- first run make noyield-test and make pthread-rwlock-test
  - Both of these tests MUST pass
  - noyield-test checks for a proproccessor flag that should only be set for local testing
  - pthread-rwlock-test checks whether they may have used the pthread_rwlock in their code
...
- Example:
  make
  make results
  ./check-result.sh phashcoarse ALL_TESTS 
