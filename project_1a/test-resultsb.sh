...
< "$rundir/tests-$prog" xargs -n1 -I'{}' bash -c "'$rundir/test-result.sh' '$prog' {} >/dev/null 2>&1 && echo '$prog $numthreads $numkeys {} $numiter: PASS' || echo '$prog $numthreads $numkeys {} $numiter: FAIL'"
